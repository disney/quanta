...
func TestMemDb(t *testing.T) {
	...
	dc.Put(context.TODO(), &datasource.KeyInt{Id: 123}, []driver.Value{123, "aaron", "email@email.com", created.In(time.UTC), []string{"admin"}})
	row, err := dc.Get(123)
	assert.Equal(t, nil, err)
	...
	_, err = dc.Put(context.TODO(), &datasource.KeyInt{Id: 225}, []driver.Value{})
	assert.NotEqual(t, nil, err)

	dc.Put(context.TODO(), &datasource.KeyInt{Id: 123}, []driver.Value{123, "aaron", "aaron@email.com", created.In(time.UTC), []string{"root", "admin"}})
	row, _ = dc.Get(123)
	assert.NotEqual(t, nil, row)
	...
	_, err = dc.PutMulti(context.TODO(), []schema.Key{&key}, [][]driver.Value{{123, "aaron", "aaron@email.com", created.In(time.UTC), []string{"root", "admin"}}})
	assert.Equal(t, nil, err)
