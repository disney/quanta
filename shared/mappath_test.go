...
func TestPath(t *testing.T) {
	...
	var val interface{}
	_, err = GetPath("metadata/1/receivedTimestamp", dat, false)
	assert.Error(t, err, "Key not present. [Key:receivedTimestamp]")
	val, err = GetPath("metadata/0/receivedTimestamp", dat, false)
	assert.NoError(t, err)
	assert.Equal(t, val, "12-14-1993")
	val, err = GetPath("data/partner", dat, false)
	assert.NoError(t, err)
	assert.Equal(t, val, "espn")
	val, err = GetPath("badpath/testname", dat, true)
	assert.NoError(t, err)
	assert.Equal(t, val, "mpath_test")
	val, err = GetPath("/badpath/testname", dat, true)
	assert.NoError(t, err)
	assert.Equal(t, val, "mpath_test")
	_, err = GetPath("/badpath/testname2", dat, true)
